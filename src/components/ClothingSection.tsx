...
import {
  ...
  clothingConditionLabels,
  categoryLabels,
  formatPrice,
} from "@/lib/sanity-types";

const categories: ClothingCategory[] = ["mujer", "hombre", "accesorios"];

interface ClothingSectionProps {
  items: SanityClothingItem[];
}

export function ClothingSection({ items }: ClothingSectionProps) {
  const [activeCategory, setActiveCategory] = useState<ClothingCategory>("mujer");
  const [selectedItem, setSelectedItem] = useState<SanityClothingItem | null>(null);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  const filteredItems = items.filter((item) => item.category === activeCategory);

  const openDrawer = (item: SanityClothingItem) => {
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          {filteredItems.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-zinc-500">No hay productos en esta categoría todavía.</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="aspect-[3/4] relative overflow-hidden bg-zinc-100">
                      {item.images?.[0] && (
                        <SanityImage
                          image={item.images[0]}
                          alt={`${item.name} usado en Caseros`}
                          fill
                      ...
                      {!isSold && (
                        ...
                        >
                          {clothingConditionLabels[item.condition]}
                        </span>
                      )}
                    </div>
                    <div className="p-4">
            ...
            <div className="relative aspect-square bg-zinc-100">
              {selectedItem.images?.[currentImageIndex] && (
                <SanityImage
                  ...
                  alt={`${selectedItem.name} usado en Caseros`}
                  fill
              ...
              >
                {clothingConditionLabels[selectedItem.condition]}
              </span>
            </div>
